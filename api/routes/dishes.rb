  ...
  route do |r|
    ...
    r.is do
      r.get do
        ...
        %w[meat vegetarian soup].each do |type|
          result[type] = Dish.where(type: type).map do |dish|
      ...
      r.post do
        ...
        # 验证必填字段
        required = %w[name description type]
        ...
        r.halt(400, { error: "无效菜品类型，有效类型：#{valid_types.join(', ')}" }) unless valid_types.include?(data['type'])
        
        # 检查菜品是否已存在
        existing = Dish.where(name: data['name'], type: data['type']).first
        ...
        { id: dish.id, name: dish.name, description: dish.description, type: dish.type }
      end
    end
    ...
    r.on Integer do |id|
      ...
      r.put do
        ...
        dish.type = data['type'] if data['type'] && %w[meat vegetarian soup].include?(data['type'])
